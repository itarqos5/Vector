...
public final class VectorConsole {
    ...
    private void process(final String input) {
        ...
        switch (command) {
            ...
            case "kick" -> {
                if (parts.length < 2) {
                    TerminalLogger.warn("Usage: kick <ip>");
                    return;
                }
            ...
            case "send" -> {
                if (parts.length < 4) {
                    TerminalLogger.warn("Usage: send <ip> <host> <port>");
                    return;
                }
                final String ip = parts[1];
                ...
                if (kicked > 0) {
                    TerminalLogger.info("Disconnected " + ip + " — will route to "
                        + destHost + ":" + destPort + " on next reconnect.");
                } else {
                    TerminalLogger.info("No active session for " + ip
                        + " — override set. Will route to " + destHost + ":" + destPort
                        + " on next connection.");
                }
            }
            ...
            case "ban" -> {
                if (parts.length < 2) {
                    TerminalLogger.warn("Usage: ban <ip>");
                    return;
                }
                final String ip = parts[1];
                banList.ban(ip);
                ...
                final int kicked = registry.kickByIp(ip);
                if (kicked > 0) {
                    TerminalLogger.info("Kicked " + kicked + " active connection(s) from " + ip);
                }
            }

            case "unban" -> {
                if (parts.length < 2) {
                    TerminalLogger.warn("Usage: unban <ip>");
                    return;
                }
                final String ip = parts[1];
                if (banList.unban(ip)) {
            ...
            case "banlist" -> {
                ...
                if (all.isEmpty()) {
                    ...
                    for (final String ip : all) {
                        TerminalLogger.info("  " + ip);
                    }
                }
            }
            ...
            default -> TerminalLogger.warn("Unknown command: \"" + command + "\". Type 'help' for command list.");
        }
    }
